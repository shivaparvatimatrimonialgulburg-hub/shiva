        ...
        <div className="flex items-center space-x-6">
          <div className="relative">
            <div className="w-32 h-32 rounded-3xl overflow-hidden border-4 border-white shadow-xl">
              <img 
                src={profile.profileImageUrl || `https://picsum.photos/seed/${profile.uid}/200/200`} 
          ...
          <div>
            <h1 className="text-3xl font-serif font-bold text-primary">{profile.fullName}</h1>
            <p className="text-muted-foreground flex items-center mt-1">
              <MapPin className="h-4 w-4 mr-1" /> {profile.nativePlace || 'Gulbarga'}, {profile.state || 'Karnataka'}
            </p>
            <div className="flex space-x-2 mt-3">
              <Badge variant="outline" className="bg-white">{profile.gender}</Badge>
              <Badge variant="outline" className="bg-white">{profile.maritalStatus}</Badge>
            </div>
          </div>
        </div>
          ...
          <Card className="border-none shadow-sm">
            ...
            <CardContent className="grid grid-cols-2 gap-y-6 gap-x-4">
              <div>
                ...
                <p className="font-medium flex items-center"><Calendar className="h-4 w-4 mr-2 text-muted-foreground" /> {profile.birthDate}</p>
              </div>
              <div>
                <p className="text-xs text-muted-foreground uppercase tracking-wider font-bold mb-1">Birth Time</p>
                <p className="font-medium flex items-center"><Clock className="h-4 w-4 mr-2 text-muted-foreground" /> {profile.birthTime || 'Not specified'}</p>
              </div>
              <div>
                <p className="text-xs text-muted-foreground uppercase tracking-wider font-bold mb-1">Education</p>
                <p className="font-medium flex items-center"><GraduationCap className="h-4 w-4 mr-2 text-muted-foreground" /> {profile.education || 'Not specified'}</p>
              </div>
              <div>
                <p className="text-xs text-muted-foreground uppercase tracking-wider font-bold mb-1">Occupation</p>
                <p className="font-medium flex items-center"><Briefcase className="h-4 w-4 mr-2 text-muted-foreground" /> {profile.occupation || 'Not specified'}</p>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="border-none shadow-sm bg-secondary/10">
            ...
            <CardContent>
              <p className="text-sm leading-relaxed text-muted-foreground italic">
                "{profile.expectations || 'No expectations specified yet.'}"
              </p>
            </CardContent>
          </Card>
